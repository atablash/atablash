...
// atablash modified version for move-only function objects
...
#	endif
	...
	template<typename Result, typename... Arguments>
	struct is_valid_function_argument<FunctionMovable<Result (Arguments...)>, Result (Arguments...)>
	{
		static const bool value = false;
	};
...
template<typename Result, typename... Arguments>
class FunctionMovable<Result (Arguments...)>
	: public detail::typedeffer<Result, Arguments...>
{
public:
	FunctionMovable() FUNC_NOEXCEPT
	{
		initialize_empty();
	}
	FunctionMovable(std::nullptr_t) FUNC_NOEXCEPT
	{
		initialize_empty();
	}
	FunctionMovable(FunctionMovable && other) FUNC_NOEXCEPT
	{
		initialize_empty();
		swap(other);
	}
	...
	FunctionMovable(T&& functor,
			typename std::enable_if<detail::is_valid_function_argument<TT, Result (Arguments...)>::value, detail::empty_struct>::type = detail::empty_struct()) FUNC_TEMPLATE_NOEXCEPT(T, std::allocator<typename detail::functor_type<TT>::type>)
	{
		...
		{
			typedef typename detail::functor_type<TT>::type functor_type;
			//LOGI("dupa");
	...
	template<typename Allocator>
	FunctionMovable(std::allocator_arg_t, const Allocator &)
	{
		// ignore the allocator because I don't allocate
		initialize_empty();
	}
	template<typename Allocator>
	FunctionMovable(std::allocator_arg_t, const Allocator &, std::nullptr_t)
	{
		// ignore the allocator because I don't allocate
		initialize_empty();
	}
	template<typename Allocator, typename T>
	FunctionMovable(std::allocator_arg_t, const Allocator & allocator, T functor,
	...
	template<typename Allocator>
	FunctionMovable(std::allocator_arg_t, const Allocator &, FunctionMovable && other) FUNC_NOEXCEPT
	{
		// ignore the allocator because I don't allocate
		initialize_empty();
		swap(other);
	...
	FunctionMovable & operator=(FunctionMovable other) FUNC_NOEXCEPT
	{
		swap(other);
		return *this;
	}
	...
	{
		FunctionMovable(std::allocator_arg, allocator, functor).swap(*this);
	}
	
	void swap(FunctionMovable & other) FUNC_NOEXCEPT
	{
...
template<typename T>
bool operator==(std::nullptr_t, const FunctionMovable<T> & rhs) FUNC_NOEXCEPT
{
	return !rhs;
}
template<typename T>
bool operator==(const FunctionMovable<T> & lhs, std::nullptr_t) FUNC_NOEXCEPT
{
	return !lhs;
}
template<typename T>
...
template<typename T>
void swap(FunctionMovable<T> & lhs, FunctionMovable<T> & rhs)
{
	lhs.swap(rhs);
}
...
struct uses_allocator<ab::FunctionMovable<Result (Arguments...)>, Allocator>
	: std::true_type
{
};
}
