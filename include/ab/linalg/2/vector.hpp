...
public:
	...
	// niech nic nie zwraca, ¿eby siê nie myli³o
...
inline Vector2<decltype(F()*G())> operator*(const F& a, const Vector2<G>& v){
	return Vector2<decltype(F()*G())>(a*v.x, a*v.y);
}
...
// ostream
template<class F>
std::ostream& operator<<(std::ostream& os, const ab::Vector2<F>& v){
	return os << "{" << v.x << ", " << v.y << "}";
}
