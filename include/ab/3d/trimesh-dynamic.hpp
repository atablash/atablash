...
class TriMeshD : public ab::Entity{
	friend TriMeshD_detail::V <      TriMeshD>;
	friend TriMeshD_detail::V <const TriMeshD>;
	friend TriMeshD_detail::Vs<const TriMeshD>;
	friend TriMeshD_detail::Vs<      TriMeshD>;
	friend TriMeshD_detail::Ps<      TriMeshD>;
	friend TriMeshD_detail::Ps<const TriMeshD>;
public:
...
//
public:
	TriMeshD_detail::Vs<const TriMeshD> vs() const;
	TriMeshD_detail::Vs<      TriMeshD> vs();

	TriMeshD_detail::V<const TriMeshD> vs(uint ith) const;
	TriMeshD_detail::V<      TriMeshD> vs(uint ith);
...
//
public:
	inline TriMeshD_detail::Ps<      TriMeshD> ps();
	inline TriMeshD_detail::Ps<const TriMeshD> ps() const;

	inline TriMeshD_detail::P<      TriMeshD> ps(uint ith);
	inline TriMeshD_detail::P<const TriMeshD> ps(uint ith) const;
...
public:
	...
	// V REMAP EXACT
	// map vertices with the same position AND attributes to the first of them
	// (pending merge)
private:
	bool using_vremapExact = false;

public:
	ab::UnionFind<ui32> vremapExact;
	void enable_vremapExact() {
		if (using_vremapExact) return;
		vremapExact.clear();
		using_vremapExact = true;
	}
	void disable_vremapExact() {
		if (!using_vremapExact) return;
		using_vremapExact = false;
		vremapExact.clear();
	}
	bool isUsing_vremapExact() const {
		return using_vremapExact;
	}
	void apply_vremapExact() {
		for (uint i = 0; i < indices.size(); ++i) {
			indices[i] = vremapExact[indices[i]];
		}
	}
...
namespace TriMeshD_detail {
	...
	typedef TriMeshD::POS_FLOAT POS_FLOAT;
	typedef TriMeshD::IDX_INT   IDX_INT;
	typedef TriMeshD::IIDX_INT  IIDX_INT;
	...
	template<> class V<const TriMeshD> {
		friend Vs<const TriMeshD>; friend Vs<TriMeshD>; friend TriMeshD; friend V<TriMeshD>;
	public:
		IDX_INT idx() const { return ith; }
	...
	private:
		V(const TriMeshD& _g, IDX_INT _ith) : g(_g), ith(_ith) {}
		const TriMeshD& g;
		const IDX_INT ith;
	};
	...
	public:
		...
		V& operator=(const V& o) {
			return this->operator=<V<TriMeshD>>(o);
		}
		...
		V& operator=(const O& o) {
			...
			// no real support for vremap! have to do unionFind.rename
			//if (g.usingVRemap) {
			//	if (&g == &o.g) g.vremap[ith] = o.g.vremap[o.ith];
				//else g.vremap[ith] = ith; // erase vremap if moving vertex from another mesh (erasing not supported!!!)
			//}
			return *this;
		}

		// copy attributes except position
		void copyAttributesFrom(V& a) {
			if (g.rgbOffset())    rgb() = a.rgb();
			if (g.alphaOffset())  alpha() = a.alpha();
			if (g.uvOffset())     uv() = a.uv();
			if (g.normalOffset()) normal() = a.normal();
			// TODO: how to avoid those ifs?
		}

		template<class F>
		V& lerpAttributes(V& a, V& b, const F& _t) {
			float t = (float)_t;
			float tt = 1.0f - t;
			// todo: what if using greater precision?
	...
	private:
		V(TriMeshD& _g, IDX_INT _ith) : g(_g), ith(_ith) {}
		TriMeshD& g;
		const IDX_INT ith;
	};
	...
	template<class TRIMESH> class Ps {
		friend TriMeshD;
	public:
	...
	template<> class P<TriMeshD> {
		friend Ps<TriMeshD>; friend TriMeshD;
	public:
		...
		V<TriMeshD> operator[](int i) {
			return g.vs(g.indices[ith * 3 + i]);
		...
		P& operator=(const P& o) {
			return this->operator=<P<TriMeshD>>(o);
		}
		template<class O>
	...
	template<> class P<const TriMeshD> {
		friend Ps<const TriMeshD>; friend Ps<TriMeshD>; friend TriMeshD; friend P<TriMeshD>;
	public:
		const ab::Vector3<IDX_INT>& idx() {
			return *(ab::Vector3<IDX_INT>*)&g.indices[ith * 3];
			//return *(ab::Vector3<idxint>*)&g.idata[ith*g.isize];
		}
		const IDX_INT& idx(int i){
			return g.indices[ith * 3 + i];
		}
		V<const TriMeshD> operator[](int i) {
			return g.vs(g.indices[ith * 3 + i]);
			//return *(IDX_INT*)&g.idata[ith*g.isize + i*sizeof(idxint)];
		}
		Triangle3<POS_FLOAT> triangle() const {
...
// vertices access
//
inline TriMeshD_detail::Vs<      TriMeshD> TriMeshD::vs()       { return TriMeshD_detail::Vs<      TriMeshD>(*this); }
inline TriMeshD_detail::Vs<const TriMeshD> TriMeshD::vs() const { return TriMeshD_detail::Vs<const TriMeshD>(*this); }

inline TriMeshD_detail::V<      TriMeshD> TriMeshD::vs(uint ith)       { return TriMeshD_detail::V<      TriMeshD>(*this, ith); }
inline TriMeshD_detail::V<const TriMeshD> TriMeshD::vs(uint ith) const { return TriMeshD_detail::V<const TriMeshD>(*this, ith); }


//
// polygons access
//
inline TriMeshD_detail::Ps<      TriMeshD> TriMeshD::ps()       { return TriMeshD_detail::Ps<      TriMeshD>(*this); }
inline TriMeshD_detail::Ps<const TriMeshD> TriMeshD::ps() const { return TriMeshD_detail::Ps<const TriMeshD>(*this); }

inline TriMeshD_detail::P<      TriMeshD> TriMeshD::ps(uint ith)       { return TriMeshD_detail::P<      TriMeshD>(*this, ith); }
inline TriMeshD_detail::P<const TriMeshD> TriMeshD::ps(uint ith) const { return TriMeshD_detail::P<const TriMeshD>(*this, ith); }
