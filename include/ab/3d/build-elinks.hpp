...
inline std::vector<ui32> buildELinks_detail(const TRIMESH& mesh, const VREMAP& vremap){
	...
	for(uint i=0; i<mesh.ps().size(); ++i){
		// optim todo: check mesh.indices(...) instead
		auto ia = mesh.ps(i).idx(0);
		auto ib = mesh.ps(i).idx(1);
		auto ic = mesh.ps(i).idx(2);
		if (useRemap){
			ia = vremap[ia];
			ib = vremap[ib];
			ic = vremap[ic];
	...
	for(uint i=0; i<mesh.ps().size(); ++i){
		// optim todo: check mesh.indices(...) instead
		int ia = mesh.ps(i).idx(0);
		int ib = mesh.ps(i).idx(1);
		int ic = mesh.ps(i).idx(2);
		if (useRemap){
			ia = vremap[ia];
			ib = vremap[ib];
			ic = vremap[ic];
