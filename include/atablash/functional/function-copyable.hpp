...
/*
You can define FUNC_NO_EXCEPTIONS to prevent the function from using exceptions.
In that case you will get a segmentation fault instead when calling an empty std::function.
You can define FUNC_NO_RTTI to compile the std::function if you have turned off RTTI in your compiler.
In that case the target_type() and target() member functions will not be available. That one came from libc++.
*/
...
#define FUNC_TEMPLATE_NOEXCEPT(FUNCTOR, ALLOCATOR) noexcept(fc_detail::is_inplace_allocated<FUNCTOR, ALLOCATOR>::value)
...
#	endif
	...
	struct is_inplace_allocated
	{
		static const bool value
			...
			//&& std::is_nothrow_move_constructible<T>::value	// TODO: don't care about exceptions?
	...
	struct functor_type
	{
		typedef typename std::remove_reference<decltype(to_functor(std::declval<T>()))>::type type;
	};
	...
	template<typename Result, typename... Arguments>
	struct is_valid_function_argument<FunctionCopyable<Result (Arguments...)>, Result (Arguments...)>
	{
		static const bool value = false;
	};
...
#		endif
	...
	// store functor in-place
	template<typename T, typename Allocator, typename Enable = void, class TT = typename std::remove_reference<T>::type>
	struct function_manager_inplace_specialization
	{
		...
		static Result call(const functor_padding & storage, Arguments... arguments)
		{
			...
			return const_cast<TT &>(reinterpret_cast<const TT &>(storage))(FUNC_FORWARD(Arguments, arguments)...);
		}

		static void store_functor(manager_storage_type & storage, T&& to_store)
		{
			//LOGI("zupa");
			new (&get_functor_ref(storage)) TT(FUNC_FORWARD(T, to_store));
		}
		
		static void move_functor(manager_storage_type & lhs, manager_storage_type && rhs) FUNC_NOEXCEPT
		{
			new (&get_functor_ref(lhs)) TT(FUNC_MOVE(get_functor_ref(rhs)));
		}
		static void destroy_functor(Allocator &, manager_storage_type & storage) FUNC_NOEXCEPT
		{
			get_functor_ref(storage).~TT();
		}
		static TT & get_functor_ref(const manager_storage_type & storage) FUNC_NOEXCEPT
		{
			return const_cast<TT &>(reinterpret_cast<const TT &>(storage.functor));
		}
	};
	
	// store functor on heap (big functors)
	template<typename T, typename Allocator>
	...
	template<typename T, typename Allocator, class TT = typename std::remove_reference<T>::type>
	static void create_manager(manager_storage_type & storage, Allocator && allocator)
	{
		...
		storage.manager = &get_default_manager<TT, Allocator>();
	}
...
#		endif
		...
		{
			//LOGD("templated_call_move_and_destroy");
...
#		endif
		...
		static void templated_call_copy_functor_only(manager_storage_type & lhs, const manager_storage_type & rhs){
			//LOGD("templated_call_copy_functor_only");
			typedef function_manager_inplace_specialization<T&, Allocator> specialization;
			specialization::store_functor(lhs, specialization::get_functor_ref(rhs));
		}
	};
...
template<typename Result, typename... Arguments>
class FunctionCopyable<Result (Arguments...)>
	: public fc_detail::typedeffer<Result, Arguments...>
{
public:
	FunctionCopyable() FUNC_NOEXCEPT
	{
		initialize_empty();
	}
	FunctionCopyable(std::nullptr_t) FUNC_NOEXCEPT
	{
		initialize_empty();
	}
	FunctionCopyable(FunctionCopyable && other) FUNC_NOEXCEPT
	{
		initialize_empty();
		swap(other);
	}
	FunctionCopyable(const FunctionCopyable & other)
		: call(other.call)
	{
		other.manager_storage.manager->call_copy(manager_storage, other.manager_storage);
	}
	template<typename T, class TT = typename std::remove_reference<T>::type>
	FunctionCopyable(T&& functor,
			typename std::enable_if<fc_detail::is_valid_function_argument<TT, Result (Arguments...)>::value, fc_detail::empty_struct>::type = fc_detail::empty_struct()) FUNC_TEMPLATE_NOEXCEPT(T, std::allocator<typename fc_detail::functor_type<TT>::type>)
	{
		if (fc_detail::is_null(functor))
		{
			initialize_empty();
		}
		else
		{
			typedef typename fc_detail::functor_type<TT>::type functor_type;
			//LOGI("dupa");
			
			initialize(fc_detail::to_functor(FUNC_FORWARD(T,functor)), std::allocator<functor_type>());
		}
	}
	template<typename Allocator>
	FunctionCopyable(std::allocator_arg_t, const Allocator &)
	{
		// ignore the allocator because I don't allocate
		initialize_empty();
	}
	template<typename Allocator>
	FunctionCopyable(std::allocator_arg_t, const Allocator &, std::nullptr_t)
	{
		// ignore the allocator because I don't allocate
		initialize_empty();
	}
	template<typename Allocator, typename T>
	FunctionCopyable(std::allocator_arg_t, const Allocator & allocator, T functor,
			typename std::enable_if<fc_detail::is_valid_function_argument<T, Result (Arguments...)>::value, fc_detail::empty_struct>::type = fc_detail::empty_struct())
			FUNC_TEMPLATE_NOEXCEPT(T, Allocator)
	{
		...
		{
			initialize(fc_detail::to_functor(FUNC_FORWARD(T, functor)), Allocator(allocator));
		}
	}
	template<typename Allocator>
	FunctionCopyable(std::allocator_arg_t, const Allocator & allocator, const FunctionCopyable & other)
		: call(other.call)
	{
		typedef typename std::allocator_traits<Allocator>::template rebind_alloc<FunctionCopyable> MyAllocator;

		// first try to see if the allocator matches the target type
		fc_detail::manager_type manager_for_allocator = &fc_detail::get_default_manager<typename std::allocator_traits<Allocator>::value_type, Allocator>();
		if (other.manager_storage.manager == manager_for_allocator)
		{
			fc_detail::create_manager<typename std::allocator_traits<Allocator>::value_type, Allocator>(manager_storage, Allocator(allocator));
		...
		{
			fc_detail::manager_type manager_for_function = &fc_detail::get_default_manager<FunctionCopyable, MyAllocator>();
			if (other.manager_storage.manager == manager_for_function)
			{
				fc_detail::create_manager<FunctionCopyable, MyAllocator>(manager_storage, MyAllocator(allocator));
	...
	template<typename Allocator>
	FunctionCopyable(std::allocator_arg_t, const Allocator &, FunctionCopyable && other) FUNC_NOEXCEPT
	{
		// ignore the allocator because I don't allocate
		initialize_empty();
		swap(other);
	...
	FunctionCopyable & operator=(FunctionCopyable other) FUNC_NOEXCEPT
	{
		swap(other);
		return *this;
	}
	...
	{
		FunctionCopyable(std::allocator_arg, allocator, functor).swap(*this);
	}
	
	void swap(FunctionCopyable & other) FUNC_NOEXCEPT
	{
		fc_detail::manager_storage_type temp_storage;
...
#		ifdef FUNC_NO_EXCEPTIONS
			return call != nullptr;
#		else
			return call != &fc_detail::empty_call<Result, Arguments...>;
#		endif
	}

private:
	fc_detail::manager_storage_type manager_storage;
	Result (*call)(const fc_detail::functor_padding &, Arguments...);

	template<typename T, typename Allocator, typename TT = typename std::remove_reference<T>::type>
	void initialize(T&& functor, Allocator && allocator)
	{
		//LOGI("srupa");
		call = &fc_detail::function_manager_inplace_specialization<T, Allocator>::template call<Result, Arguments...>;
		fc_detail::create_manager<T, Allocator>(manager_storage, FUNC_FORWARD(Allocator, allocator));
		fc_detail::function_manager_inplace_specialization<T, Allocator>::store_functor(manager_storage, FUNC_FORWARD(T, functor));
	}

	typedef Result(*Empty_Function_Type)(Arguments...);
	void initialize_empty() FUNC_NOEXCEPT
	{
		//LOGI("initialize_empty");
		typedef std::allocator<Empty_Function_Type> Allocator;
		static_assert(fc_detail::is_inplace_allocated<Empty_Function_Type, Allocator>::value, "The empty function should benefit from small functor optimization");

		fc_detail::create_manager<Empty_Function_Type&, Allocator>(manager_storage, Allocator());
		fc_detail::function_manager_inplace_specialization<Empty_Function_Type, Allocator>::store_functor(manager_storage, nullptr);
#		ifdef FUNC_NO_EXCEPTIONS
			call = nullptr;
#		else
			call = &fc_detail::empty_call<Result, Arguments...>;
#		endif
	}
};

template<typename T>
bool operator==(std::nullptr_t, const FunctionCopyable<T> & rhs) FUNC_NOEXCEPT
{
	return !rhs;
}
template<typename T>
bool operator==(const FunctionCopyable<T> & lhs, std::nullptr_t) FUNC_NOEXCEPT
{
	return !lhs;
}
template<typename T>
...
template<typename T>
void swap(FunctionCopyable<T> & lhs, FunctionCopyable<T> & rhs)
{
	lhs.swap(rhs);
}
...
struct uses_allocator<ab::FunctionCopyable<Result (Arguments...)>, Allocator>
	: std::true_type
{
};
}
