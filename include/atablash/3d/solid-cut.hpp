...
inline TriCutResult triCut(TRIMESH& mesh, uint poly, const Vector3<typename TRIMESH::POS_FLOAT>& V){
	...
	ASS(V != mesh.vs(mesh.indices[poly * 3 + 0]).pos());
	ASS(V != mesh.vs(mesh.indices[poly * 3 + 1]).pos());
	ASS(V != mesh.vs(mesh.indices[poly * 3 + 2]).pos());
	...
	mesh.ps()[BCV][0] = mesh.ps()[poly][1];
	mesh.ps()[BCV][1] = mesh.ps()[poly][2];
	mesh.ps()[BCV][2] = V_idx;
	//ASS(isPolyOk(mesh, BCV));
	
	mesh.ps()[CAV][0] = mesh.ps()[poly][2];
	mesh.ps()[CAV][1] = mesh.ps()[poly][0];
	mesh.ps()[CAV][2] = V_idx;
...
EdgeCutResult edgeCut(TRIMESH& mesh, uint edge,
	...
	mesh.ps().push_back();
	mesh.ps()[VBC][0] = V_idx;
	mesh.ps()[VBC][1] = mesh.indices[VC];
	mesh.ps()[VBC][2] = mesh.indices[CA];
	...
	if (VA != (ui32)(-1)){
		...
		bool doubleVertices = mesh.indices[AD] != mesh.indices[edge] || mesh.ps()[VBC][1] != mesh.indices[VA];
		...
		r.DBV = DBV;
		mesh.ps().push_back();
		mesh.ps()[DBV][0] = mesh.indices[DV];
		mesh.ps()[DBV][1] = mesh.indices[VA];
		mesh.ps()[DBV][2] = V2_idx;
