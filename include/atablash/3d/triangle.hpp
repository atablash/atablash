...
public:
	...
	bool isDegenerate() const {
		...
		if ((BA ^ CA).lengthSqr() < Triangle3Eps<TFLOAT>().degenerate){
...
template<class F, class EPS = Triangle3Eps<F>>
inline CutInfo<F> triangle3Intersection(const Triangle3<F>& t1, const Triangle3<F>& t2,
	...
	// odleg³oœci drugiego od p³aszczyzny pierwszego
	F d2a = n1*t2.a + d1;
	F d2b = n1*t2.b + d1;
	F d2c = n1*t2.c + d1;
	...
	// prosta L przeciêcia p³aszczyzn trójk¹tów (jej kierunek)
	Vector3<F> D = n1 ^ n2;	// TODO: mo¿na spróbowaæ nie liczyæ tego, tylko bezpoœrednio porównaæ, wzd³u¿ których osi s¹ n1 i n2
