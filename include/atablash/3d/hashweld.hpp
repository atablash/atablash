...
inline HashWeldResult hashWeldExact(const TRIMESH& mesh){
	...
	typedef Vector3<TRIMESH::POS_FLOAT> Key;
...
inline void hashWeldExactOptimize(TRIMESH& mesh)
{
	...
	typedef p<Vector3<TRIMESH::POS_FLOAT>,Vector2<float>> Key;
...
inline void hashWeldOptimize(TRIMESH& mesh,
	const typename TRIMESH::POS_FLOAT& weldPosEpsilon = (TRIMESH::POS_FLOAT)0.001,
...
{
	typedef p<Vector3<i32>,Vector2<i32>> Key;
	umap<Key,int> weldMap;
	
	TRIMESH::POS_FLOAT weldPosEpsilonRec = 1 / weldPosEpsilon;
